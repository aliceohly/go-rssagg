...
const getLastFeeds = `-- name: GetLastFeeds :many
SELECT id, name, url, created_at, updated_at, user_id, last_fetched_at
FROM feeds
...
func (q *Queries) GetLastFeeds(ctx context.Context, limit int32) ([]Feed, error) {
	rows, err := q.db.QueryContext(ctx, getLastFeeds, limit)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
